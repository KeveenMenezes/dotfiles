...
update ()
{
  ...
  # Sempre buscar informações da música atual, tocando ou não
  TRACK="$(echo "$INFO" | jq -r .Name | sed 's/\(.\{20\}\).*/\1.../')"
  ARTIST="$(echo "$INFO" | jq -r .Artist | sed 's/\(.\{20\}\).*/\1.../')"
  ALBUM="$(echo "$INFO" | jq -r .Album | sed 's/\(.\{25\}\).*/\1.../')"
  
  # Se tiver uma música válida, mostrar informações
  if [ -n "$TRACK" ] && [ "$TRACK" != "null" ]; then
    SHUFFLE=$(osascript -e 'tell application "Spotify" to get shuffling')
    REPEAT=$(osascript -e 'tell application "Spotify" to get repeating')
    COVER=$(osascript -e 'tell application "Spotify" to get artwork url of current track')
    
    # Só baixar a capa se for uma música diferente
    CURRENT_TRACK_FILE="/tmp/spotify_current_track.txt"
    LAST_TRACK=""
    if [ -f "$CURRENT_TRACK_FILE" ]; then
      LAST_TRACK=$(cat "$CURRENT_TRACK_FILE")
    fi
    
    if [ "$TRACK - $ARTIST" != "$LAST_TRACK" ]; then
      curl -s --max-time 20 "$COVER" -o /tmp/cover.jpg
      echo "$TRACK - $ARTIST" > "$CURRENT_TRACK_FILE"
    fi
    ...
    # Animar o aparecimento do label suavemente
    sketchybar --animate tanh 30 --set spotify.anchor label.drawing=on label="$TRACK - $ARTIST"
  else
    # Sem música - manter ícone visível mas sem label
    sketchybar --animate tanh 30 --set spotify.anchor label.drawing=off popup.drawing=off
  fi
}
